...
### 2026-03-27 - 课程功能优化
1. **飞行课扫帚选择功能修复**：
   - 修复了扫帚选择界面不显示的问题（之前只在拥有多个扫帚时才显示）
   - 现在即使只有普通扫帚也会显示选择界面，让玩家了解这个功能
   - 受影响文件：`flying-class.html`

2. **咒语课功能优化**：
   - 修复了答案正确也显示发音不对的问题（优化了空格处理和匹配逻辑）
   - 修改了咒语书倒计时：从30秒改为15秒
   - 优化了咒语书倒计时逻辑：关闭后再打开不会重置，每次换咒语后重新倒计时，开始挑战后才开始倒计时
   - 新增功能：倒计时结束后无法再查看咒语书（按钮禁用+样式变化+警告提示）
   - 修复了未定义函数错误：删除了 `checkAndUnlockCharmsAchievements` 的调用
   - 受影响文件：`charms-class.html`

3. **魔药课功能优化**：
   - 新增功能：倒计时也显示在材料选择页面（之前只在配方页面显示）
   - 新增功能：倒计时结束后无法再查看配方（按钮禁用+样式变化+警告提示）
   - 受影响文件：`potions-class.html`
